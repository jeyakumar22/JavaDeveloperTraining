...
public class AccountService {
    ...
    private String generateAccountCode(AccountType type) {
        ...
        Optional<Account> lastAccountOpt = accountRepository.findTopByTypeOrderByAccountCodeDesc(type);

        int newCode;

        if (lastAccountOpt.isEmpty()) {

            newCode = prefix + 1;
        } else {
            String lastCodeStr = lastAccountOpt.get().getAccountCode();
